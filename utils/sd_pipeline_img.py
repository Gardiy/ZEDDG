...
class ZEDDGStableDiffusionPipeline(DiffusionSeecoderPipeline):
    @torch.no_grad()
    def __call__(
        self,
        ...
        self.check_inputs(
            height, width, callback_steps, negative_prompt, prompt_embeds, negative_prompt_embeds
        )
        ...
        zepa = torch.zeros_like(prompt_embeds)
        lista_de_tensores = [prompt_embeds]  + [zepa]* 3
        prompt_embeds = torch.concat((lista_de_tensores), dim=0) #([4, 77, 768])
        # 4. Prepare timesteps
